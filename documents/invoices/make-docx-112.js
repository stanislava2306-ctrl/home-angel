...
const parties = table([new TableRow({ children: [
  ...
  cell([
    label("Bill to"),
    p(run("Morin Michel", { size: 17, bold: true }), { after: 30 }),
    body("4 R\u00e9sidence Beausoleil"),
    body("14400 Saint-Vigor-le-Grand, France"),
  ], { w: 34, mr: 300 }),
  cell([
    p(run("Invoice", { size: 12, bold: true, ls: 32, caps: true }), { after: 60, align: AlignmentType.RIGHT }),
    p(run("#112", { size: 24, bold: true }), { after: 60, align: AlignmentType.RIGHT }),
    metaRow("Invoice date", "01.09.2026"),
    metaRow("Due date", "16.09.2026"),
    metaRow("Currency", "EUR (€)"),
  ], { w: 31 }),
]})]);

/* ---------- items table ---------- */
const COLS = [26, 42, 9, 10, 13];
...
const items = [
  ["Discovery & Specification", "Requirements workshop, specification and sitemap", "8", "€65.00", "€520.00"],
  ["UX Architecture", "User flows, wireframes and responsive page structure", "10", "€70.00", "€700.00"],
  ["UI Design System", "Design system, components and final page layouts", "12", "€75.00", "€900.00"],
  ["Front-End Development", "HTML / CSS / JS build and cross-browser testing", "14", "€62.50", "€875.00"],
  ["Back-End & Integrations", "Server-side logic, CMS, forms and API integrations", "8", "€85.00", "€680.00"],
  ["SEO, QA & Deployment", "SEO, performance tuning, QA, hosting setup and launch", "10", "€52.50", "€525.00"],
];
...
const grand = table([new TableRow({ children: [
  ...
  cell([p(run("€5,000.00", { size: 30, bold: true }), { align: AlignmentType.RIGHT })], { w: 54, mt: 150, mb: 40, va: VerticalAlign.BOTTOM, borders: { top: b(LINE, 6), bottom: NONE, left: NONE, right: NONE } }),
]})]);
...
const summary = table([new TableRow({ children: [
  cell([
    p([
      ...
      run("All items above form one indivisible engagement — the full-cycle development and launch of the Client's website — and are billed together as a single deliverable. Total 62 hours.", { size: 13, color: MUTE }),
    ], { line: 250 }),
  ], { w: 55, mr: 500, mt: 200 }),
  cell([totLine("Subtotal", "€4,200.00"), totLine("VAT 20%", "€840.00"), totLine("Total incl. VAT", "€5,040.00"), totLine("Discount", "\u2212€40.00"), grand], { w: 45, mt: 120 }),
]})]);
...
Packer.toBuffer(doc).then(buf => {
  fs.writeFileSync("Invoice_112_Fourtress_Commerce_Ltd.docx", buf);
